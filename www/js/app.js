...
function onDeviceReady() {
    ...
    if (getStorage('dir') == "rtl") {
        $('link[href="css/app-rtl.css"]').prop('disabled', false);
		

    } else {
        $('link[href="css/app-rtl.css"]').prop('disabled', true);
		
    }
            window.Freshchat.init({
    appId       : "679820cf-24ef-4220-b9bf-e54f9de55687",
    appKey      : "173bb8e4-f3f0-4eea-8e3b-2b0a38a6addc"
  });
...
function createElement2(elementId, elementvalue) {
    var content = document.getElementById(elementId);
    content.innerHTML = elementvalue;
    ons.compile(content);
}
        ...
        case "page-home":
            ...
            if (search_mode == "postcode") {
                ...
                if (enabled_advance_search == "yes") {
                    ...
                    var tab_html = '';
                    tab_html += '<ons-tabbar position="bottom" class="menu_tabbar" var="menu_tabbar" >';
                    if (theme_address != 2) {
            ...
            if (data.code == 1) {
                switch (action) {
                    ...
                    case "saveSettings":
                        if (isLogin()) {
                            onsenAlert(data.msg);
                            var content = document.getElementById('kSettingsNavigator');
                            content.innerHTML = "";
                            ons.compile(content);
                            getLanguageSettings();
                        } else {
                    ...
                    case "getLanguageSettings":
                        ...
                        var forcelang = getStorage("langid");
                        ...
                        if (!empty(forcelang)) {
                            device_set_lang = getStorage("langid");
                            setStorage("default_lang", forcelang);
                        }
                        ...
                        setLanguage(getStorage('langid'));
                        var options = {
                            animation: 'slide',
                            onTransitionEnd: function() {
                                if (getStorage('dir') == "rtl") {
                                    var htm = "<ons-sliding-menu main-page='home.html'  menu-page='menu.html' side='right' var='menu' close-on-tap type='push' ></ons-sliding-menu>";
                                    createElement2('page-slidingMenu', htm);
                                    $('link[href="css/app-rtl.css"]').prop('disabled', false);
									

                                } else {
                                    var htm = "<ons-sliding-menu main-page='home.html'  menu-page='menu.html' side='left' var='menu' close-on-tap type='push' ></ons-sliding-menu>";
                                    createElement2('page-slidingMenu', htm);
                                    $('link[href="css/app-rtl.css"]').prop('disabled', true);
...
function displayRestaurantResults(data, target_id) {
    ...
    $.each(data, function(key, val) {
        ...
        htm += '</ons-row>';
        htm += '</ons-list-item>';
        htm += '<ons-list style="border-top: none"><ons-list-item style="line-height: 1; padding: 0;">';
        htm += ' <ons-row class="action">';
        htm += ' <ons-col class="action-col">';
        htm += ' <div class="action-icon"><img src="css/images/icons/' + val.tag_raw + '.png"></div>';
        htm += '<div class = "action-label">' + val.is_open + '</div>';
        htm += '</ons-col>';
        dump(val.services);
        if (!empty(val.services)) {
            $.each(val.services, function(key_service, val_services) {
                htm += '<ons-col class="action-col">';
                htm += '<div class="action-icon"><img src="css/images/icons/tick.png"/></div>';
                htm += ' <div class="action-label">' + val_services + ' </div>';
                htm += ' </ons-col>';
            });
        }
        ...
        if (!empty(val.payment_available)) {
            if (val.payment_available.length > 0) {
                $.each(val.payment_available, function(key_pv, val_pv) {
                    htm += '<ons-col class="action-col">';
                    htm += '<div class="action-icon"><img src="css/images/icons/bill.png"></ons-icon></div>';
                    htm += '<div class = "action-label">' + val_pv + '</div>';
                    htm += '</ons-col>';
                });
            }
        }
        htm += ' </ons-row>';
        htm += ' </ons-list-item></ons-list>';
    });
...
function onRequestSuccess() {
    ...
    navigator.geolocation.getCurrentPosition(geolocationSuccess, geolocationError, { timeout: 10000, enableHighAccuracy: getLocationAccuracy() });
...
function setLanguage(lang_id) {
    if (isLogin()) { //removeStorage("translation");
        ...
        setStorage('langid', lang_id);
        setStorage('dir', getTrans('ltr', 'csst'));
    }
}
...
function loadPageMerchantInfo() {
    ...
    if (typeof merhantPopOverMenu === "undefined" || merhantPopOverMenu == null || merhantPopOverMenu == "") {} else {
...
function applyCashTip() {
    ...
    if (!empty(order_sub_total)) {
        ...
        dump(reverse_percentage.toFixed());
...
function initSlideMenu() {
    menu.on('preopen', function() {
        ...
        if (isLogin()) {
            ...
            if (!empty(avatar)) {
                dump('fillavatar');
                $(".bar-profile").css({ "display": "block" });
                $(".profile-picture").attr("src", getStorage("avatar"));
                $(".name").html(getStorage("client_name_cookie"));
            }
        } else {
            dump('logon not');
            $(".logout-menu").hide();
